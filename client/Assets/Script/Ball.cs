...
public class Ball : MonoBehaviour 
{
	...
	{
		if(col.name.Equals("net") && !trigger_col) //ball <-> net
		{
			
		}
		if(col.name.Equals("Player1")&&!trigger_col) // ball <-> player
		{
			is_spiked = false;
			Player1 p = col.gameObject.GetComponent<Player1>();
			if(!p.upperSpike && !p.middleSpike && !p.lowerSpike)
			{
				vel_x = vel_x/10 + p.vel_x/200 + DirVectorElement(ball.transform.localPosition.x, p.transform.localPosition.x-10);
				if(ball.transform.localPosition.y < p.transform.localPosition.y)
					vel_y = -vel_y + p.vel_y/200;
				else
					vel_y = -vel_y + p.vel_y/200 + DirVectorElement(ball.transform.localPosition.y, p.transform.localPosition.y);
				trigger_col = true;	
			}
